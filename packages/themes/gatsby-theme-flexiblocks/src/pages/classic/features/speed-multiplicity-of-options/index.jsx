...
const IndexPage = props => {
  ...
  return (
    <Layout theme={theme} {...props} xs={styles.body}>
      ...
      <Seo title='Email Campaign Options | Import Contacts | MumaraClassic' description="Explore variety of email campaign options to customize the sending pattern for better delivery. Import Contacts speedily to contact list instead of adding one by one." keywords="send email,email campaign options,speedily import contacts,email from,custom email" />
        {/* Blocks */}
...
export const query = graphql`
  query classicFeaturesPage12BlockContent {
    allBlockContent(
      filter: { page: { in: ["classic/features/speed-multiplicity-of-options", "shared"] } }
    ) {
      nodes {
        ...BlockContent
      }
