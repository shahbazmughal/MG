...
const IndexPage = props => {
  ...
  return (
    <Layout theme={theme} {...props} xs={styles.body}>
      ...
      <Seo title='Free Email Verifier | Email Checker | MumaraClassic' description="MumaraClassic email checker is free email verifier that would help you run multilevel email verification process to check if the email is valid." keywords="free email verifier,email checker,email validation,email verification tool,verify email address,bulk email verifier" />
        {/* Blocks */}
...
export const query = graphql`
  query classicFeaturesPage11BlockContent {
    allBlockContent(
      filter: { page: { in: ["classic/features/maintain-clean-email-list", "shared"] } }
    ) {
      nodes {
        ...BlockContent
      }
