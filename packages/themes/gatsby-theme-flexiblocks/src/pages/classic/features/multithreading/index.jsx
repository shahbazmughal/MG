...
const IndexPage = props => {
  ...
  return (
    <Layout theme={theme} {...props} xs={styles.body}>
      ...
      <Seo title='Multiple Email Threads | Multithreading | MumaraClassic' description="Run multiple email threads to achieve certain throughput by supplying emails in parallel connection, and reduce the sending time to a large extent." keywords="email threads,mail thread,multithreading,parallel sending activity,speedy sending,reduce sending time" />
        {/* Blocks */}
...
export const query = graphql`
  query classicFeaturesPage6BlockContent {
    allBlockContent(
      filter: { page: { in: ["classic/features/multithreading", "shared"] } }
    ) {
      nodes {
        ...BlockContent
      }
