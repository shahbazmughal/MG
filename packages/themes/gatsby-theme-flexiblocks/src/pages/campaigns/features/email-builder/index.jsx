...
const campFeatures08 = props => {
  const { allBlockContent } = props.data
  const content = normalizeBlockContentNodes(allBlockContent?.nodes)

  return (
    <Layout theme={theme} {...props}>
      <Seo title='Drag and Drop Email Builder | Campaigns' description="" keywords=""/>
      {/* Blocks */}
...
export const query = graphql`
  query campFeatures08BlockContent {
    allBlockContent(
      filter: { page: { in: ["campaigns/features/email-builder", "shared"] } }
    ) {
      nodes {
        ...BlockContent
      }
