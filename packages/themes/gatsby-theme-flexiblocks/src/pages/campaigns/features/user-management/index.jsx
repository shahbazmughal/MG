...
const campFeaturesUserManagement = props => {
  ...
  return (
    <Layout theme={theme} {...props} xs={styles.body}>
      ...
      <Seo title='Easy Contact Management | Import Contacts' description="Store contact details keep it safe and updated within the contact lists, use stable import function to import sizeable amount of contacts with ease." keywords="contact management system,custom contacts,contact details for email,update contact details,import contacts" />
        ...
        <div id="user-roles" className="innerpage-section bg-white img-right">
          <FeatureLeft content={content['features1']} />
        </div>
        <div id="packages" className="innerpage-section bg-light img-left">
          <FeatureRight content={content['features2']} />
        </div>
        <div id="user-accounts" className="innerpage-section bg-white img-right">
...
export const query = graphql`
  query campFeaturesUserManagementBlockContent {
    allBlockContent(
      filter: { page: { in: ["campaigns/features/user-management", "shared"] } }
    ) {
      nodes {
        ...BlockContent
      }
...
`
export default campFeaturesUserManagement
